   ...
   public void myEncode(Picture messagePict)
   {
 	  ...
 	  for (int row = 0; row < this.getHeight(); row++)
 	  {
 		  for (int col = 0; col < this.getWidth(); col++)
 		  {
 			  ...
 			  int blue = ((int)(Math.pow((currPixel.getRed()%10), 2) + (int)(Math.pow((currPixel.getGreen()% 10), 2))));
 			  //encode
			  messagePixel = messagePixels[row][col];
			  if (messagePixel.colorDistance(Color.BLACK) < 40)
			  {
				  currPixel.setBlue((currPixel.getBlue()-(currPixel.getBlue()%10)) + (blue%10));
		
			  }
			  else if ((((int)(Math.pow((currPixel.getRed()%10), 2) + (int)(Math.pow((currPixel.getGreen()% 10), 2)))%10) == (currPixel.getBlue()%10)))
			  {
				  currPixel.setBlue(currPixel.getBlue()+ 8);
			  }
 		  }
 	  }
 	  System.out.println(count);
   ...
   public Picture myDecode()
   {
 	  ...
 	  for (int row = 0; row < this.getHeight(); row++)
 	  {
 		  for (int col = 0; col < this.getWidth(); col++)
 		  {
 			  ...
 			  messagePixel = messagePixels[row][col];
 			  //decode
 			 if (((int)(Math.pow((currPixel.getRed()%10), 2) + (int)(Math.pow((currPixel.getGreen()% 10), 2)))% 10 == (currPixel.getBlue()%10)))
 			 {
				  messagePixel.setColor(Color.BLACK);
				  count++;
 			 }
...
//left arm row: 156-193 col: 99-168
//right arm row: 163-200 col: 238-296
  }
